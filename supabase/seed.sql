...
-- Re-insert default strike settings (truncated above)
...
-- ===================================================================
-- PART 1B: Create Auth Identities (Required for login!)
-- ===================================================================
...
DECLARE
  user_record RECORD;
BEGIN
  RAISE NOTICE 'Creating auth identities for email provider...';

  -- For each user, create a corresponding identity
  FOR user_record IN
  ...
  RAISE NOTICE 'Auth identities created! Users can now log in.';
END $$;
...
-- Scenario 1: Game in PRIORITY_OPEN phase (3 weeks out - Tuesday)
...
VALUES (
  ...
  -- Next Tuesday + 3 weeks (always a Tuesday)
  CURRENT_DATE + INTERVAL '1 day' * (CASE
    WHEN EXTRACT(DOW FROM CURRENT_DATE) < 2 THEN (2 - EXTRACT(DOW FROM CURRENT_DATE)::INTEGER + 21)
    ELSE (9 - EXTRACT(DOW FROM CURRENT_DATE)::INTEGER + 21)
  END),
  'priority_open',
  NOW() + INTERVAL '2 days',
  NOW() + INTERVAL '4 days',
...
-- Scenario 2: Game in WAITLIST_OPEN phase (2 weeks out - Tuesday)
INSERT INTO games (id, game_date, status, priority_deadline, payment_reminder_time, payment_deadline, sunday_lottery_deadline, teams_announced)
VALUES (
  '00000000-0000-0000-0000-000000000002',
  -- Next Tuesday + 2 weeks (always a Tuesday)
  CURRENT_DATE + INTERVAL '1 day' * (CASE
    WHEN EXTRACT(DOW FROM CURRENT_DATE) < 2 THEN (2 - EXTRACT(DOW FROM CURRENT_DATE)::INTEGER + 14)
    ELSE (9 - EXTRACT(DOW FROM CURRENT_DATE)::INTEGER + 14)
  END),
  'waitlist_open',
  NOW() - INTERVAL '1 day',
  NOW() + INTERVAL '2 days',
...
-- Scenario 3: Game in PAYMENT_PENDING phase (1 week out - Tuesday)
INSERT INTO games (id, game_date, status, priority_deadline, payment_reminder_time, payment_deadline, sunday_lottery_deadline, teams_announced)
VALUES (
  '00000000-0000-0000-0000-000000000003',
  -- Next Tuesday + 1 week (always a Tuesday)
  CURRENT_DATE + INTERVAL '1 day' * (CASE
    WHEN EXTRACT(DOW FROM CURRENT_DATE) < 2 THEN (2 - EXTRACT(DOW FROM CURRENT_DATE)::INTEGER + 7)
    ELSE (9 - EXTRACT(DOW FROM CURRENT_DATE)::INTEGER + 7)
  END),
  'payment_pending',
  NOW() - INTERVAL '3 days',
  NOW() - INTERVAL '1 hour',
...
-- Scenario 4: Game with TEAMS_ASSIGNED (this coming Tuesday)
INSERT INTO games (id, game_date, status, priority_deadline, payment_reminder_time, payment_deadline, sunday_lottery_deadline, teams_announced)
VALUES (
  '00000000-0000-0000-0000-000000000004',
  -- Next Tuesday from today (always a Tuesday)
  CURRENT_DATE + INTERVAL '1 day' * (CASE
    WHEN EXTRACT(DOW FROM CURRENT_DATE) < 2 THEN (2 - EXTRACT(DOW FROM CURRENT_DATE)::INTEGER)
    ELSE (9 - EXTRACT(DOW FROM CURRENT_DATE)::INTEGER)
  END),
  'teams_assigned',
  NOW() - INTERVAL '5 days',
  NOW() - INTERVAL '3 days',
...
BEGIN
  ...
  SELECT COUNT(*) INTO identity_count FROM auth.identities WHERE provider = 'email' AND identity_data->>'email' LIKE '%@dropin.test';
  ...
  RAISE NOTICE 'Auth identities created: %', identity_count;
  ...
  RAISE NOTICE 'Ready for login at http://localhost:3000/login';
  RAISE NOTICE '=========================';
END $$;
